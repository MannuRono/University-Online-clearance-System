...
if ($result->num_rows > 0) {
    ...
    $checkQuery = "SELECT * FROM registry_cleared WHERE regno='$regno'";
    $checkResult = $db->query($checkQuery);

    if ($checkResult->num_rows > 0) {
        $message = "<span style='color: red;'>The student has already been cleared.</span>";
    } else {
        // Insert the student's details into the registry_declined table
        $insertQuery = "INSERT INTO registry_declined (regno, name, department, faculty, gender, email) VALUES ('$regno', '$name', '$department', '$faculty', '$gender', '$email')";
        if ($db->query($insertQuery) === true) {
            // Delete the student record from registry_clearancerequest table
            $deleteQuery = "DELETE FROM registry_clearancerequest WHERE regno='$regno'";
...
<html>
<head>
    <title>registry Declined</title>
</head>
<body>
    <h1>registry Declined</h1>
