...
if ($result->num_rows > 0) {
    ...
    $checkQuery = "SELECT * FROM clubs_cleared WHERE regno='$regno'";
    $checkResult = $db->query($checkQuery);

    if ($checkResult->num_rows > 0) {
        $message = "<span style='color: red;'>The student has already been cleared.</span>";
    } else {
        // Insert the student's details into the clubs_declined table
        $insertQuery = "INSERT INTO clubs_declined (regno, name, department, faculty, gender, email) VALUES ('$regno', '$name', '$department', '$faculty', '$gender', '$email')";
...
<html>
<head>
    <title>Clubs Declined</title>
</head>
<body>
    <h1>Clubs Declined</h1>
