...
if ($result->num_rows > 0) {
    ...
    $checkQuery = "SELECT * FROM registry_declined WHERE regno='$regno'";
    $checkResult = $db->query($checkQuery);

    if ($checkResult->num_rows > 0) {
        $message = "<span style='color: red;'>The student has already been declined.</span>";
    } else {
        // Insert the student's details into the registry_cleared table
        $insertQuery = "INSERT INTO registry_cleared (regno, name, department, faculty, gender, email) VALUES ('$regno', '$name', '$department', '$faculty', '$gender', '$email')";
...
<html>
<head>
    <title>registry Cleared</title>
</head>
<body>
    <h1>registry Cleared</h1>
