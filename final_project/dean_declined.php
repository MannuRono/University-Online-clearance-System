...
if ($result->num_rows > 0) {
    ...
    $checkQuery = "SELECT * FROM dean_cleared WHERE regno='$regno'";
    $checkResult = $db->query($checkQuery);

    if ($checkResult->num_rows > 0) {
        $message = "<span style='color: red;'>The student has already been cleared.</span>";
    } else {
        // Insert the student's details into the dean_declined table
        $insertQuery = "INSERT INTO dean_declined (regno, name, department, faculty, gender, email) VALUES ('$regno', '$name', '$department', '$faculty', '$gender', '$email')";
